---
source_path: Prompt/API/API_TD_2_Method_Header_BreakDown.txt
...
# DEFAULT          : Sinh case gọi sai HTTP Method
# ON               : Tương đương DEFAULT
# OFF              : Bỏ qua toàn bộ kiểm thử HTTP Method

CONTENT_TYPE_CHECK : [DEFAULT]
# Giá trị hợp lệ  : DEFAULT | ON | OFF
# DEFAULT          : Sinh case sai Content-Type
# ON               : Tương đương DEFAULT
# OFF              : Bỏ qua toàn bộ kiểm thử Content-Type
```

> **Lưu ý:** [Security] Authorization và [Basic] Custom Headers KHÔNG có tham số kiểm soát — BẮT BUỘC sinh đầy đủ mọi trường hợp, không được bỏ qua.
...
- Bước 0: Đọc CẤU HÌNH KIỂM SOÁT CẤU PHẦN ở trên. Ghi nhớ giá trị `METHOD_CHECK` và `CONTENT_TYPE_CHECK`.
...
- Bước 2: **[Protocol]** — Áp dụng `METHOD_CHECK`:
  + `DEFAULT` hoặc `ON`: Sinh case gọi sai HTTP Method (Thường trả về HTTP 405 Method Not Allowed hoặc 404).
  + `OFF`: Bỏ qua, chuyển sang Bước 3.
- Bước 3: **[Security]** — BẮT BUỘC. Duyệt phần Authorization (nếu tài liệu PTTK yêu cầu API này phải có Token/Auth). Sinh các case: Thiếu Token (401), Token sai format (401), Token hết hạn (401), Token không có quyền truy cập API này (403).
- Bước 4: **[Format]** — Áp dụng `CONTENT_TYPE_CHECK`:
  + `DEFAULT` hoặc `ON`: Sinh case sai Media Type (Thường trả về HTTP 415 Unsupported Media Type).
  + `OFF`: Bỏ qua, chuyển sang Bước 5.
- Bước 5: **[Basic]** — BẮT BUỘC. Duyệt các Custom Header khác (nếu có). Sinh case Missing và Invalid Format cho các header đó.
...
1. Tự Self-Audit: Có test condition nào đang test một field trong Request Body không? (Nếu có -> Xóa ngay). Có dòng nào verify DB thay đổi không? (Nếu có -> Xóa ngay). Có bỏ sót case [Security] hoặc [Basic] không? (Nếu có -> Bổ sung ngay).
